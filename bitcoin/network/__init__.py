...
testnet_params = {
    'magic': codecs.decode(b'0B110907', 'hex'),
    'port': 18333,
}

regtest_params = {
    'magic': codecs.decode(b'FABFB5DA', 'hex'),
    'port': 18444,
}
...
class GeventConnection(Connection):
    ...
    def read_message(self):
        ...
        message = self.parse_message(command.decode('utf-8'), payload)
        return message
...
class ClientBehavior(object):
    ...
    def on_version(self, connection, unused_message):
        if connection.incoming:
            self.send_version(connection)
            time.sleep(3)
            logging.info("Sending verack now")
